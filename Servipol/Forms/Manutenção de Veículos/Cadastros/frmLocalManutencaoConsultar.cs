...
namespace Servipol.Forms.Manutenção_de_Veículos.Cadastros
{
    public partial class frmLocalManutencaoConsultar : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void CarregaTabelaLocalManutencao()
        {
            ...
            {
                BD.Conectar();
                NpgsqlDataAdapter retornoBD = new NpgsqlDataAdapter($"", BD.ObjetoConexao);
        ...
        private void btnInativar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string sqlCommand = $"UPDATE manutencao_local SET ativo = 'N', id_usuario_desativacao = {SessaoSistema.UsuarioId}, data_desativacao = CURRENT_TIMESTAMP WHERE id_manutencao_local = {idRegistroSelecionadoGrid}";
        ...
        private void btnConsultar_Click(object sender, EventArgs e)
        {
            ...
            switch (cBoxTipoBusca.SelectedIndex)
            {
                case 0:
                    ...
                    break;
                case 1:
                    tipoBusca = $"lt.descricao ILIKE '%{tBoxTextoConsulta.Text.ToUpper().Trim()}%'";
                    break;
                default:
                    tipoBusca = "1=1";
                    break;
            ...
            {
                BD.Conectar();
                NpgsqlDataAdapter retornoBD = new NpgsqlDataAdapter($" WHERE mt.ativo = '{situacaoTraduzida}' AND {tipoBusca} ORDER BY ordem ASC", BD.ObjetoConexao);
                DataTable dp = new DataTable();
                retornoBD.Fill(dp);
                dGridLocalManutencao.DataSource = dp;
            }
